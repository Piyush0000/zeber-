      ...
      <section className="bg-gradient-to-br from-blue-50 via-white to-blue-50 py-32 px-6">
        <div className="container mx-auto text-center max-w-4xl">
          <h1 className="text-4xl md:text-6xl font-bold text-blue-900 mb-8 animate-fade-in-down">
            Our <span className="bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent animate-rainbow">AI Services</span>
          </h1>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <div 
                key={index} 
                className="bg-white border border-blue-200 rounded-2xl p-8 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 animate-fade-in-up"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="text-6xl mb-6 text-center text-blue-600 animate-pulse">{service.icon}</div>
                <h3 className="text-2xl font-bold text-blue-900 mb-4 text-center animate-fade-in-up">{service.title}</h3>
                <p className="text-blue-700 mb-8 text-center animate-fade-in-up delay-100">{service.description}</p>
      ...
      <section className="py-24 px-6 bg-gradient-to-br from-blue-50 to-white">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-bold text-blue-900 mb-6 animate-fade-in-up">Our Best Projects</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {projects.map((project, index) => (
              <div 
                key={index} 
                className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 animate-fade-in-up border border-blue-200"
                style={{ animationDelay: `${index * 150}ms` }}
              >
                <div className="h-48 bg-gradient-to-r from-blue-600 to-blue-800 flex items-center justify-center">
                  <span className="text-5xl text-white font-bold animate-pulse">{project.name.charAt(0)}</span>
                </div>
                <div className="p-8">
                  <h3 className="text-2xl font-bold text-blue-900 mb-4 animate-fade-in-up">{project.name}</h3>
                  <p className="text-blue-700 mb-6 animate-fade-in-up delay-100">{project.description}</p>
                  <div className="mb-8">
                    <span className="inline-block bg-gradient-to-r from-blue-100 to-blue-50 text-blue-800 text-sm font-medium px-4 py-2 rounded-full animate-fade-in">
                      {project.client}
                    </span>
                  </div>
